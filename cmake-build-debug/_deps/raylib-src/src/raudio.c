...
void UpdateMusicStream(Music music)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        if ((music.stream.buffer != NULL) && !music.stream.buffer->isSubBufferProcessed[i]) continue; // No refilling required, move_step to next sub-buffer
                ...
                while (framesToRead > 0)
                {
                    ...
                    if (framesJustRead < framesToReadRightNow)
                    {
                        ...
                        else
                        {
                            ...
                            // move_step the cursor position back to the start and continue the loop
                            audioBuffer->frameCursorPos = 0;
                            continue;
                        }
                    }
