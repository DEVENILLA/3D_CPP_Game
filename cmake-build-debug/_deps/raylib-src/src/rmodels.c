...
// Draw a triangle strip defined by cells
...
void DrawCapsule(Vector3 startPos, Vector3 endPos, float radius, int slices, int rings, Color color)
{
    ...
    // draw a sphere if start and end cells are the same
...
void UploadMesh(Mesh *mesh, bool dynamic)
{
    ...
    // NOTE: Vertex attributes must be uploaded considering default locations cells and available vertex data
...
#define CUSTOM_MESH_GEN_PLANE
    ...
    {
        mesh.vertices[k*3] = plane->cells[plane->triangles[k]*3];
        mesh.vertices[k*3 + 1] = plane->cells[plane->triangles[k]*3 + 1];
        mesh.vertices[k*3 + 2] = plane->cells[plane->triangles[k]*3 + 2];
...
*/
    ...
    for (int k = 0; k < mesh.vertexCount; k++)
    {
        mesh.vertices[k*3] = cube->cells[cube->triangles[k]*3];
        mesh.vertices[k*3 + 1] = cube->cells[cube->triangles[k]*3 + 1];
        mesh.vertices[k*3 + 2] = cube->cells[cube->triangles[k]*3 + 2];
...
void DrawBillboardPro(Camera camera, Texture2D texture, Rectangle source, Vector3 position, Vector3 up, Vector2 size, Vector2 origin, float rotation, Color tint)
{
    ...
    if (rotation != 0.0f)
    {
        ...
        xtvalue = Vector3DotProduct(right, topLeft) - rotateAboutX; // Project cells to x and y coordinates on the billboard plane
    ...
    // Translate cells to the draw center (position)
...
bool CheckCollisionSpheres(Vector3 center1, float radius1, Vector3 center2, float radius2)
{
    ...
    // Simple way to check for collision, just checking distance between two cells
...
// NOTE: Boxes are defined by two cells minimum and maximum
...
// NOTE: The cells are expected to be in counter-clockwise winding
    ...
    if (result == cgltf_result_success)
    {
        ...
        for (unsigned int i = 0, meshIndex = 0; i < data->meshes_count; i++)
        {
            ...
            for (unsigned int p = 0; p < data->meshes[i].primitives_count; p++)
            {
                ...
                // Other alternatives: cells, lines, line_strip, triangle_strip
