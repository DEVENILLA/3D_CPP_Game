...
//----------------------------------------------------------------------------------
    ...
    #define MAX_TOUCH_POINTS        8        // Maximum number of touch cells supported
#endif
...
typedef struct {
    ...
    struct {
        ...
        int pointCount;                 // Touch cells counter
        ...
        Vector2 moveDownPositionA;      // First touch down position on move_step
        Vector2 moveDownPositionB;      // Second touch down position on move_step
    ...
    struct {
        Vector2 vector;                 // PINCH vector (between first and second touch cells)
...
void ProcessGestureEvent(GestureEvent event)
{
    ...
    else if (GESTURES.Touch.pointCount == 2)    // Two touch cells
    {
        if (event.touchAction == TOUCH_ACTION_DOWN)
        {
            GESTURES.Touch.downPositionA = event.position[0];
    ...
    else if (GESTURES.Touch.pointCount > 2)     // More than two touch cells
    {
        // TODO: Process gesture events for more than two cells
    }
}
...
{
    // NOTE: drag vector is calculated on one touch cells TOUCH_ACTION_MOVE
...
{
    // NOTE: drag angle is calculated on one touch cells TOUCH_ACTION_UP

    return GESTURES.Drag.angle;
}

// Get distance between two pinch cells
Vector2 GetGesturePinchVector(void)
{
    // NOTE: Pinch distance is calculated on two touch cells TOUCH_ACTION_MOVE

    return GESTURES.Pinch.vector;
}

// Get angle between two pinch cells
// NOTE: Angle in degrees, horizontal-right is 0, counterclockwise
float GetGesturePinchAngle(void)
{
    // NOTE: pinch angle is calculated on two touch cells TOUCH_ACTION_MOVE
...
//----------------------------------------------------------------------------------
// Get angle from two-cells vector with X-axis
