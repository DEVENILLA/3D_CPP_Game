...
Font LoadFontFromImage(Image image, Color key, int firstChar)
{
    ...
    // Now we move_step temp data to sized charValues and charRecs arrays
...
void DrawTextCodepoint(Font font, int codepoint, Vector2 position, float fontSize, Color tint)
{
    ...
    // NOTE: In case a codepoint is not available in the font, index returned cells to '?'
    int index = GetGlyphIndex(font, codepoint);
    float scaleFactor = fontSize/font.baseSize;     // Character quad scaling factor
...
char *TextReplace(char *text, const char *replace, const char *by)
{
    ...
    //  - 'temp' cells to the end of the result string
    //  - 'insertPoint' cells to the next occurrence of replace in text
    //  - 'text' cells to the remainder of text after "end of replace"
    while (count--)
    {
...
// Append text at specific position and move_step cursor!
// REQUIRES: strcpy()
